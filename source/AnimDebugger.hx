...
class AnimDebugger extends MusicBeatState
{
	...
	var stage:Stage;
	var bgLayer:FlxTypedGroup<Stage>;
	...
	override function create()
	{
		...
		bgLayer = new FlxTypedGroup<Stage>();
		add(bgLayer);
	...
	function reloadBGs() {
		...
		while(i >= 0) {
			var memb:Stage = bgLayer.members[i];
			if(memb != null) {
				memb.kill();
				bgLayer.remove(memb);
				memb.destroy();
	...
	function addCharacterUI() {
		...
		noAntialiasingCheckBox.callback = function() {
			...
			if(!noAntialiasingCheckBox.checked && OptionHandler.aliasing) {
